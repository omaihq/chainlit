...
import {
  ...
  ITasklistElement,
  IThread
} from '@/react-client/types';
import {
  addMessage,
  deleteMessageById,
  updateMessageById,
  updateMessageContentById
} from '@/react-client/utils/message';
...
const useChatSession = () => {
  ...
  useEffect(() => {
    if (session?.socket) {
      (session.socket.auth as any)['threadId'] = currentThreadId || '';
    }
  }, [currentThreadId]);
    ...
    async ({
      ...
      socket.on('audio_connection', async (state: 'on' | 'off') => {
        if (state === 'on') {
          ...
          await wavRecorder.record(async (data: any) => {
